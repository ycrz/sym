<?php
    ...
    if (isset($_POST['tatl']) && isset($_POST['name']) && isset($_POST['amount']) && isset($_POST['wallet'])){
    	$arr = ['BNI','BRI','MANDIRI','PERMATA'];
    	if (!in_array($_POST['wallet'], $arr)) {
    		echo json_encode(array('tmp' => "Tipe pembayaran harus BNI, BRI, MANDIRI atau PERMATA. <br>Hubungi tim admin kami bila mengalami kesulitan.", "flag" => 'false'));
            die();
    	}else{
    		$res=api();
    	}
