<?php 
    ...
    function proceed($conn){
        $fid=$_GET['fid'];
        $res = queryBack("SELECT fa.name,bo.trx_number,bp.va_number,bp.media,bo.deposit,bo.status FROM whatsapp.billing_order bo JOIN whatsapp.billing_payment bp ON bp.billing_order_id=bo.id JOIN singa_yehuda_mengaum.form_attendee fa ON fa.id=bo.uid WHERE bo.trx_number like '010002%'");
        $arr = [];
        for ($i=0; $i < $res[0]; $i++) {
            $status = '';
